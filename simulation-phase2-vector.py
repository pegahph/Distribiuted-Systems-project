...
def mesg(th_no, port, port_count):
    ...
    for port in Packet_string["clock"].keys():
        if port not in vector_clock.keys():
            vector_clock[port] = 0
        vector_clock[port] = max(Packet_string["clock"][port], vector_clock[port])
