import socket 
import threading
import random
import time


Packet_string=""


thread_count=15
max_nei=150
NeiCout=[]
sum_nei=0
difvalue=0
counter=0
port_start=20000


for i in range(0,thread_count):
      NeiCout.append(random.randint(1, thread_count-1))
      sum_nei+=NeiCout[i]

if max_nei > sum_nei:
      difvalue=1
else:
      difvalue=-1

while max_nei != sum_nei :
      if NeiCout[counter]<thread_count-1 and NeiCout[counter]+difvalue>=1:
            NeiCout[counter]+=difvalue
            sum_nei+=difvalue
            if counter < thread_count-1:
                  counter+=1
            else:
                  counter=0
      elif counter < thread_count-1:
            counter+=1
      else:
                  counter=0
           


def mesg(th_no, port, port_count):
    print(f"thread no.{th_no}: Starting...\n")
    Neighbours_port=[]
    prob_gossip=0.75     
    port_list = [i for i in range(port_start, port_start+thread_count)]
    port_list.remove(port)
    Neighbours_port=random.sample(port_list,port_count)
    gossip= int((1-prob_gossip)*port_count)
    if gossip!=0:
        final_neighbor_list= random.sample(Neighbours_port, k=gossip)
    elif gossip==0:
          final_neighbor_list= random.sample(Neighbours_port, k=1)

    print(f"neighbours of port {port} : {final_neighbor_list} ")

    

    while True:

        server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        server.bind(('127.0.0.1',port))
        server.listen()
        clientsocket, address = server.accept()
        Packet_string = clientsocket.recv(1024).decode()
        print(f"port{port}:Received New Message: {Packet_string}\n")

        

        c = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        
        temp_list=final_neighbor_list
        for i in range(0,len(temp_list)):
            c.connect(('127.0.0.1',temp_list[i]))
            print(f"th_no{th_no}:sendig {Packet_string} to port_num: {temp_list[i]} ")
            c.send(bytes(Packet_string.encode()))
            
              


for i in range(0,thread_count):
    t = threading.Thread(target=mesg , args=(i,port_start+i, NeiCout[i]))
    t.start()


#while True:
time.sleep(3)

Packet_string= input("Enter your Message: ") 
send_port= int(input(f"Which port do you want to send your message to? choose between {port_start}-{port_start+thread_count-1}: "))
  
first_client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
first_client.connect(('127.0.0.1',send_port))
first_client.send(bytes(Packet_string.encode()))



